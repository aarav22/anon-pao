...
from .utils import  python_aes # custom import for blind certificate protocol
...
class RecordLayer(object):
    ...
    def calculateMAC(self, mac, seqnumBytes, contentType, data, isFinal = True):
        ...
        mac.update(compatHMAC(bytearray([(len(data) + 45 + 32 + 5) // 256 if not isFinal else len(data) //256])))  # 1 byte  133 = 96 + 32 + 5; 96 = 51 + 29 + 3 + 13
        mac.update(compatHMAC(bytearray([(len(data) + 45 + 32 + 5) % 256 if not isFinal else len(data) %256])))  # 1 byte
    ...
    def blindCertProtocol(self, initialData, mac, seqnumBytes, contentType):
        ...
        @profile
        def twoPHmac(initialData):
            """ DATA GENERATION """

            # 29 bytes string: M_p2
            n = 29
            m_p2 = ''.join(random.choices(string.ascii_letters + string.digits, k=n)).encode('ascii')

            # 9 bytes string: M_s1
            n = 3
            m_s1 = ''.join(random.choices(string.ascii_letters + string.digits, k=n)).encode('ascii')

            # 7 bytes string: M_s2
            n = 13
            m_s2 = ''.join(random.choices(string.ascii_letters + string.digits, k=n)).encode('ascii')
            ...
            data = initialData
            # pad data such that (len(data) + 13) is a multiple of 64
            if (len(data) + 13) % 64 != 0:
                data += b' ' * (64 - ((len(data) + 13) % 64))
            
            # just update the current vals in the mac
            self.calculateMAC(mac, seqnumBytes, contentType, data, isFinal=False)

            h = mac._inner.h

            # join m_p2 and m_s1 to h_serialized
            send_material = { # Point: Leakage of information
                'm_p2': m_p2,
                'm_s1': m_s1,
                'h': h
            }

            # serialize send_material
            send_material_serialized = pickle.dumps(send_material)

            # send h to proxy server
            self.sock.socket.send(send_material_serialized)

            # some delay: # Point: Some delay?
            # import time
            # time.sleep(5)

            # receive the new h from the proxy server
            rcvd_material_serialized = self.sock.socket.recv(1024)
            rcvd_material = pickle.loads(rcvd_material_serialized)


            # update the mac with the new h
            mac._inner.h = rcvd_material['h']
            mac._inner.mlen += 64 # len of m_p2 + len of m_s1 + len of m_star
            h = mac._inner.h

            mac.update(compatHMAC(m_s2)) 

            data += m_p2

            mac.update(compatHMAC(bCRLF + b"." + bCRLF))
            macBytes =  bytearray(mac.digest())

            return data, macBytes, m_s1, m_s2
        
        @profile
        # Now 2p-cbc:
        def twoPCBC(data, macBytes, m_s1, m_s2):
            ...
            if self._writeState.encContext:
                ...
                if self._writeState.encContext.isBlockCipher:
                    ...
                    # data = self.addPadding(data) ; Don't need to add padding for 2p-cbc (for this particular data)
                ...
                send_material = {
                    'enc_data': enc_data,
                    'aes_chainaing': aes_chainaing,
                }

                send_material_serialized = pickle.dumps(send_material)

                # send this aes_chainaing to the proxy server
                print("sending aes_chainaing to proxy server")
                self.sock.socket.send(send_material_serialized)

                # receive the new aes_chainaing from the proxy server

                import subprocess
                print(aes_key.hex(), len(aes_key.hex()))
                out = subprocess.getoutput("./bin/test_aes 1 12345 00000000000000000000000000000000"+aes_key.hex().upper())
                print(out)
                out = subprocess.getoutput(f"./bin/test_aes 1 12345 00000000000000000000000000000000"+aes_key.hex().upper())

                rcvd_material_serialized = self.sock.socket.recv(1024)
                rcvd_material = pickle.loads(rcvd_material_serialized)
                aes_chainaing = rcvd_material['new_chain_block']

                print(f'received aes_chainaing from proxy server: {aes_chainaing}')

                try:
                    # encrypt the remaining data with the new aes_chainaing
                    self._writeState.encContext.IV = aes_chainaing
                    data = m_s1 + m_s2 + bCRLF + b"." + bCRLF + macBytes
                    data = self.addPadding(data)

                    enc_data = self._writeState.encContext.encrypt(data)

                    # send the encrypted data to the proxy server
                    print("sending enc_data to proxy server")
                    send_material = {
                        'enc_data': enc_data
                    }
                    send_material_serialized = pickle.dumps(send_material)
        ...
        data, macBytes, m_s1, m_s2 = twoPHmac(initialData)
        twoPCBC(data, macBytes, m_s1, m_s2)
