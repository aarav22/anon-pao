...
class RecordSocket(object):
    ...
    def __init__(self, sock):
        ...
        with open('config.json', 'r') as f:
            ...
            self.num_chals = config["num_chals"]
        ...
        if self.mode == "youchoose" and self.cipher == "aead":
            self.aesgcm_pkts = [b'', b'']
            self.s_otc = otc.send()
    ...
    def send(self, msg, padding=0):
        ...
        # begin custom
        if self.drop_mode and self.cipher == "aead":
            try:
                if self.first_drop:
                    self.aesgcm_pkts[0] = data

                else:
                    if self.drop_counter < 80:
                        reply = self.s_otc.reply(self.otc_selections[self.drop_counter], self.aesgcm_pkts[0], data)
                    else:             
                        reply = self.s_otc.reply(self.otc_selections[self.drop_counter], data, self.aesgcm_pkts[0])

                    self.drop_counter += 1

                    self.data_sent += len(b'REPLYBEGIN' + pickle.dumps(reply))
                    for result in self._sockSendAll(b'REPLYBEGIN' + pickle.dumps(reply)):
                        yield result
       ...
       ## custom code: only for TLS 1.3
        if self.drop_mode:         
            if self.first_drop:
                ...
                self.first_drop = False
                self._recordSocket.first_drop = True

            else:
                seqNumBytes = self._writeState.getSeqNumBytes()

                self.first_drop = True
                self._recordSocket.first_drop = False
