...
class AESGCM(object):
    ...
    def __init__(self, key, implementation, rawAesEncrypt):
        ...
        self.key = key
        self.crypto_aesgcm = AESGCM2(key)
    ...
    def seal(self, nonce, plaintext, data):
        ...
        # custom code
        ct = self.crypto_aesgcm.encrypt(nonce, bytes(plaintext), bytes(data))
    ...
    def open(self, nonce, ciphertext, data):
        ...
        # custom code
        try:
            plaintext = self.crypto_aesgcm.decrypt(nonce, bytes(ciphertext), bytes(data))
