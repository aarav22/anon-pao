    ...
    def _sendMsg(self, msg, randomizeFirstBlock=True, update_hashes=True):
        ...
        if self.mode == "youchoose":
            ...
            if constants.bSUBJECT in original_buf:
                ...
                if self.optimze:
                    ...
                    # store every thing before and including begin_challenge:
                    hdr = original_buf[:original_buf.find(begin_challenge) + len(begin_challenge)]
                    # store every thing after till end_challenge:
                    challenges = original_buf[original_buf.find(begin_challenge) + len(begin_challenge):original_buf.find(end_challenge)]

                    # send the header:
                    msgFragment = Message(contentType, hdr)
                    for result in self._sendMsgThroughSocket(msgFragment):
                        yield result

                    # send the each character of the challenge:
                    for c in challenges:
                        c = b'0' if c == 49 else b'1'
                        msgFragment = Message(contentType, c)
                        for result in self._sendMsgThroughSocket(msgFragment):
                            yield result

                    # send the end_challenge:
                    msgFragment = Message(contentType, end_challenge)
                    for result in self._sendMsgThroughSocket(msgFragment):
                        yield result
        ...
        if self.mode == "youchoose":
            if constants.bSUBJECT in original_buf:
                ...
                self._recordLayer._recordSocket.drop_mode = False
