...
# https://github.com/tlsfuzzer/tlslite-ng/blob/74853f854edaa654923fd93bb3eaf727f8c70010/tlslite/tlsrecordlayer.py
# + YouChooseMessageProcessingExtension for handling YouChoose protocol syntax
# + optional override of self._user_record_limit
# + define _handle_custom_message_processing
# + call _handle_custom_message_processing in _sendMsg()
...
# Optional imports for message processing
try:
    import json
    import constants as custom_constants
    _MESSAGE_PROCESSING_AVAILABLE = True
except ImportError:
    _MESSAGE_PROCESSING_AVAILABLE = False


class YouChooseMessageProcessingExtension:
    """Optional extension for custom message processing during TLS communication"""
    
    def __init__(self, config_file='tmp/config.json'):
        self.enabled = False
        self.mode = "standard"
        self.cipher = "cbc"
        self.optimize = False
        ...
        if not _MESSAGE_PROCESSING_AVAILABLE:
            return
            
        try:
            with open(config_file, 'r') as f:
                config = json.load(f)
                self.enabled = config.get("mode") == "youchoose"
                self.mode = config.get("mode", "standard")
                self.cipher = config.get("cipher", custom_constants.CONFIG_CBC)
                self.optimize = config.get("optimize", False)
        except (FileNotFoundError, json.JSONDecodeError):
            pass  # Use defaults

    def get_blocks_to_modify(self):
        with open(self.config_file, 'r') as f:
            config = json.load(f)
            return config.get("blocks_to_modify", [])
    
    def should_process_subject_record(self, buffer, content_type):
        """Check if this record contains subject and should trigger special processing"""
        if not (self.enabled and content_type == ContentType.application_data):
            return False
        return custom_constants.bSUBJECT in buffer
    
    def process_optimized_challenges(self, original_buffer, content_type):
        """Process message in optimized challenge mode"""
        if not self.optimize:
            return None
            
        begin_challenge = b'BEGIN PAO CHALLENGE\r\n'
        end_challenge = b'\r\n.\r\n'
        
        if (begin_challenge not in original_buffer 
            or end_challenge not in original_buffer):
            return None
            
        begin_pos = original_buffer.find(begin_challenge)
        end_pos = original_buffer.find(end_challenge)
        
        header = original_buffer[:begin_pos + len(begin_challenge)]
        challenges = original_buffer[begin_pos + len(begin_challenge):end_pos]
        footer = end_challenge
        
        # Convert challenges to binary format
        processed_challenges = []
        int_value_of_zero_in_ascii = 48
        for c in challenges:
            processed_challenges.append( # assuming challenges are 0 or 1
                b'0' if c == int_value_of_zero_in_ascii else b'1')
 
        return {
            'header': header,
            'challenges': processed_challenges,
            'footer': footer
        }
...
class TLSRecordLayer(object):
    ...
    def __init__(self, sock):
        ...
        # Initialize message processing extension
        self.message_processor = YouChooseMessageProcessingExtension()
        self._in_http_session = False
        if self.message_processor.enabled:
            self._user_record_limit = custom_constants.TLS_RECORD_SIZE_LIMIT  # Custom limit for message processing
    ...
    def _handle_custom_message_processing(self, buf, contentType):
        """Handle custom message processing for both text and MIME emails:
        - Text emails: subject record + challenge sequence
        - MIME emails: headers + base64 attachment blocks for selective dropping
        """
        original_buf = buf
        
        # Check if this is an HTTP request (for Twitter API)
        if self._is_http_request(buf):
            self._in_http_session = True
            for result in self._handle_http_fragmentation(buf, contentType):
                yield result
            return

        # Check if we're continuing an HTTP session (this is the body)
        if self._in_http_session:
            for result in self._handle_http_fragmentation(buf, contentType):
                yield result
            self._in_http_session = False  # Reset flag after body
            return
    
        # Check if this message should trigger special processing
        if self.message_processor.should_process_subject_record(buf, contentType):
            
            if self.message_processor.optimize:
                # Handle optimized mode (text only for now)
                processed = self.message_processor.process_optimized_challenges(
                    original_buf, contentType)
                if processed:
                    # Send header
                    msgFragment = Message(contentType, processed['header'])
                    for result in self._sendMsgThroughSocket(msgFragment):
                        yield result
                    
                    # Activate drop mode and send control signal
                    self._recordLayer._recordSocket.selective_drop.drop_mode_active = True
                    self._recordLayer._recordSocket.selective_drop.send_drop_control_signal(
                        self.sock.socket, custom_constants.DROP_MSG_BEGIN
                    )
                    
                    # Send individual challenges
                    for challenge in processed['challenges']:
                        msgFragment = Message(contentType, challenge)
                        for result in self._sendMsgThroughSocket(msgFragment):
                            yield result
                    
                    # Send footer and deactivate drop mode
                    buf = processed['footer']
                    self._recordLayer._recordSocket.selective_drop.drop_mode_active = False
                    self._recordLayer._recordSocket.selective_drop.send_drop_control_signal(
                        self.sock.socket, custom_constants.DROP_MSG_END
                    )
                    self.message_processor.enabled = False
                    
            else:
                # Handle structured mode - check if this is MIME or text
                mime_data = self._parse_mime_attachment(original_buf)
                
                if mime_data and mime_data['has_attachment']:
                    # MIME email with attachments
                    for result in self._handle_mime_selective_dropping(mime_data):
                        yield result
                    return  # Early return - MIME processing handles everything
                else:
                    # Standard text email processing (existing logic)
                    # Send subject record as is (without making it part of the challenges)
                    subjectRecordSize = self.recordSize
                    newB = buf[:subjectRecordSize]
                    buf = buf[subjectRecordSize:]

                    msgFragment = Message(contentType, newB)
                    for result in self._sendMsgThroughSocket(msgFragment):
                        yield result

                    # Activate drop mode after sending subject record
                    self._recordLayer._recordSocket.selective_drop.drop_mode_active = True
                    self._recordLayer._recordSocket.selective_drop.send_drop_control_signal(
                        self.sock.socket, custom_constants.DROP_MSG_BEGIN
                    )

        # Standard fragmentation for remaining data (text mode only)
        ...
        # Send final fragment and deactivate drop mode if it was a subject message
        
        if self.message_processor.should_process_subject_record(
            original_buf, contentType
        ):
            self._recordLayer._recordSocket.selective_drop.drop_mode_active = False
            self._recordLayer._recordSocket.selective_drop.send_drop_control_signal(
    ...
    def _is_http_request(self, buf):
        """Check if buffer contains an HTTP request"""
        return buf.startswith(b'POST ') or buf.startswith(b'GET ') or \
            buf.startswith(b'PUT ') or buf.startswith(b'DELETE ')

    def _handle_http_fragmentation(self, buf, contentType):
        """Fragment HTTP messages at character level for YouChoose"""
        if not self.message_processor.enabled:
            while len(buf) > self.recordSize:
                newB = buf[:self.recordSize]
                buf = buf[self.recordSize:]
                msgFragment = Message(contentType, newB)
        ...
        # YouChoose mode: character-level fragmentation
        # Split at each character for maximum challenge positions
        
       # Check if this is HTTP headers (contains request line and headers)
        if buf.startswith(b'POST ') or buf.startswith(b'GET ') or \
        buf.startswith(b'PUT ') or buf.startswith(b'DELETE '):
            # This is headers, send normally
            msgFragment = Message(contentType, buf)
            for result in self._sendMsgThroughSocket(msgFragment):
                yield result
            return
        ...
        # Parse JSON to extract tweet text
        try:
            import json
            body_str = buf.decode('utf-8')
            ...
            # Find the tweet text in the JSON string
            text_key_pos = body_str.find('"text"')
            if text_key_pos == -1:
                msgFragment = Message(contentType, buf)
                for result in self._sendMsgThroughSocket(msgFragment):
                    yield result
            ...
            value_start = body_str.find(':', text_key_pos) + 1
            while body_str[value_start] in ' \t\n\r':
                value_start += 1
            value_start += 1  # Skip opening quote
            
            # Find closing quote
            value_end = body_str.find('"', value_start)
            
            # Three parts: before text, text, after text
            before = body_str[:value_start].encode('utf-8')
            text = body_str[value_start:value_end].encode('utf-8')
            after = body_str[value_end:].encode('utf-8')
            
            # Send before text
            msgFragment = Message(contentType, before)
            for result in self._sendMsgThroughSocket(msgFragment):
                yield result

            # Activate selective dropping for text only
            self._recordLayer._recordSocket.selective_drop.drop_mode_active = True
            self._recordLayer._recordSocket.selective_drop.send_drop_control_signal(
                self.sock.socket, custom_constants.DROP_MSG_BEGIN
            )
            ...
            for byte in text:
                msgFragment = Message(contentType, bytes([byte]))
                for result in self._sendMsgThroughSocket(msgFragment):
                    yield result
            ...
            # Send after text
            msgFragment = Message(contentType, after)
            for result in self._sendMsgThroughSocket(msgFragment):
                yield result
    ...
    def _parse_mime_attachment(self, buf):
        """Simple MIME parser that extracts attachment for block-by-block processing."""
        try:
            # Convert buffer to string
            if isinstance(buf, (bytes, bytearray)):
                email_content = buf.decode('utf-8', errors='ignore')
            else:
                email_content = str(buf)
            
            # Look for base64 attachment marker
            base64_marker = 'Content-Transfer-Encoding: base64'
            if base64_marker not in email_content:
                return None
                
            # Find the start of base64 content (after empty line following headers)
            base64_start = email_content.find(base64_marker)
                
            # Find the actual start of base64 data (after the header block)
            lines = email_content[base64_start:].split('\n')
            base64_data_start = None
            
            for i, line in enumerate(lines):
                if line.strip() == '':  # Empty line marks end of headers
                    base64_data_start = base64_start + len('\n'.join(lines[:i+1])) + 1
                    break
                    
            if base64_data_start is None:
                return None
                
            # Find the end of base64 data (next boundary or end of content)
            remaining_content = email_content[base64_data_start:]
            base64_end = len(email_content)
            
            # Look for MIME boundary that ends the attachment
            if '--' in remaining_content:
                boundary_pos = remaining_content.find('\r\n--')
                if boundary_pos != -1:
                    base64_end = base64_data_start + boundary_pos
                
            # Extract the three parts
            pre_attachment = email_content[:base64_data_start].encode('utf-8')
            base64_content = email_content[base64_data_start:base64_end]
            post_attachment = email_content[base64_end:].encode('utf-8')

            # Split base64 content into TLS record-sized blocks
            block_size = self.recordSize
            base64_blocks = []

            offset = 0
            while offset < len(base64_content):
                block_end = min(offset + block_size, len(base64_content))
                block = base64_content[offset:block_end]
                base64_blocks.append(block.encode('ascii'))  
                offset = block_end
            ...
            return {
                ...
                'has_attachment': len(base64_blocks) > 0
            }
    ...
    def _handle_mime_selective_dropping(self, mime_data):
        """Simple handling - send pre/post as-is, blocks individually."""
        
        # 1. Send everything before attachment (non-droppable)
        if mime_data['pre_attachment']:
            pre_msg = Message(ContentType.application_data, mime_data['pre_attachment'])
            for result in self._sendMsgThroughSocket(pre_msg):
                yield result
        
        # 2. Activate selective dropping for base64 blocks
        self._recordLayer._recordSocket.selective_drop.drop_mode_active = True
        self._recordLayer._recordSocket.selective_drop.send_drop_control_signal(
            self.sock.socket, custom_constants.DROP_MSG_BEGIN, 
            self.message_processor.get_blocks_to_modify()
        )
        
        # 3. Send each base64 block individually (droppable)
        for block in mime_data['base64_blocks']:
            block_msg = Message(ContentType.application_data, block)
            for result in self._sendMsgThroughSocket(block_msg):
                yield result
        
        # 4. Deactivate selective dropping
        self._recordLayer._recordSocket.selective_drop.drop_mode_active = False
        self._recordLayer._recordSocket.selective_drop.send_drop_control_signal(
            self.sock.socket, custom_constants.DROP_MSG_END
        )
        
        # 5. Send everything after attachment (non-droppable)
        if mime_data['post_attachment']:
            post_msg = Message(ContentType.application_data, mime_data['post_attachment'])
            for result in self._sendMsgThroughSocket(post_msg):
                yield result
        self.message_processor.enabled = False
    ...
    def _sendMsg(self, msg, randomizeFirstBlock=True, update_hashes=True):
        ...
        # Optional (YouChoose) message processing for selective dropping
        if self.message_processor.enabled:
            for result in self._handle_custom_message_processing(buf, contentType):
                yield result
            return
