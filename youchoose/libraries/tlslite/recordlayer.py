...
# https://github.com/tlsfuzzer/tlslite-ng/blob/5056319ea9f8b8b2205df83b50c1356277aec2f2/tlslite/recordlayer.py
# + YouChooseRecordLayerExtension class to process dropping of tls records
# + call in process_aead_send to use OT to send TLS records 
# in send() if AEAD mode is enabled
# + call get_custom_seqnum to get sequence number in _macThenEncrypt()
#  and _encryptThenSeal()
...
# Optional selective dropping imports - only imported if feature is enabled
try:
    import logging
    ...
    _SELECTIVE_DROPPING_AVAILABLE = True
except ImportError:
    _SELECTIVE_DROPPING_AVAILABLE = False

log = logging.getLogger(__name__) if _SELECTIVE_DROPPING_AVAILABLE else None


class YouChooseRecordLayerExtension:
    """Optional extension for selective TLS record dropping"""
    
    def __init__(self, config_file='tmp/config.json'):
        self.custom_writer = Writer()
        self.enabled = False
        self.mode = "standard"
        self.cipher = "cbc"
        self.num_chals = 160
        ...
        self.data_sent = 0
        self.drop_mode_active = False
        self.blocks_to_modify = []
        
        # Pre-cache the reply begin constant to avoid attribute lookup
        self._reply_begin_bytes = None
        
        if not _SELECTIVE_DROPPING_AVAILABLE:
            return
        ...
        try:
              with open(config_file, 'r') as f:
                ...
                self.num_chals = config.get("num_chals", 160)
                if self.enabled and self.cipher == custom_constants.CONFIG_AEAD:
                    self._init_aead_mode()
                    # Cache the constant to avoid repeated attribute lookups
                    self._reply_begin_bytes = custom_constants.OT_REPLY_BEGIN_MARKER
        except (FileNotFoundError, json.JSONDecodeError, KeyError):
            if log:
                log.warning("Selective drop config not found, feature disabled")

    def set_blocks_to_modify(self, blocks_to_modify):
        self.blocks_to_modify = blocks_to_modify
    
    def _should_process_record_with_ot(self):
        """
        Check if the current record should be processed with OT.
        
        IMPORTANT: This method accounts for the fact that record_counter is incremented
        in get_custom_seqnum() before process_aead_send() is called.
        
        Returns:
            tuple: (should_process, current_idx, previous_idx)
        """
        current_record_idx = self.record_counter - 1  # The record we're currently processing
        previous_record_idx = self.record_counter - 2  # The previous record (for pairing)
        
        should_process = (current_record_idx in self.blocks_to_modify 
                         or previous_record_idx in self.blocks_to_modify)
        
        return should_process
    
    def _init_aead_mode(self):
        """Initialize AEAD mode for selective dropping"""
        try:
            self.first_record_aead = None
            self.s_otc = otc.send()
            
            with open('assets/keys/s_public.pem', 'rb') as f:
                self.s_otc.public = pickle.load(f)
            with open('assets/keys/s_secret.pem', 'rb') as f:
                self.s_otc.secret = pickle.load(f)
            with open("assets/keys/selections.pem", 'rb') as f:
                self.otc_selections = pickle.load(f)
        except (FileNotFoundError, pickle.UnpicklingError):
            self.enabled = False
            if log:
                log.error("Failed to load AEAD keys, selective dropping disabled")
    
    def send_drop_control_signal(self, socket_obj, signal_type, blocks_to_modify=None):
        """Send control signals to proxy for drop mode coordination"""
        if not self.enabled:
            return
            
        try:
            if signal_type == "begin":
                socket_obj.send(custom_constants.CHALLENGE_DROP_BEGIN)
                if blocks_to_modify is not None:
                    self.set_blocks_to_modify(blocks_to_modify)
                    socket_obj.send(pickle.dumps(blocks_to_modify))
                if log:
                    log.debug("Sent drop mode BEGIN signal")
            elif signal_type == "end":
                socket_obj.send(custom_constants.CHALLENGE_DROP_END)
                if log:
                    log.debug("Sent drop mode END signal")
        except socket.error as e:
            if log:
                log.error(f"Failed to send drop control signal: {e}")

    def process_aead_send(self, data, sock_send_all_func):
        """Process data for AEAD selective dropping using oblivious transfer
        
        This method implements a two-record pairing system for selective dropping:
        1. First record is stored temporarily without sending
        2. Second record triggers OT (Oblivious Transfer) protocol with both records
        3. The OT reply is sent instead of the original TLS records
        
        This allows a receiver to selectively choose which of the two records
        to decrypt while the sender doesn't know which was chosen.
        
        IMPORTANT: record_counter is incremented in get_custom_seqnum() before this
        method is called, so we need to account for that offset when checking
        blocks_to_modify indices.
        
        Returns:
            False: Not handling this data (use normal send)
            True: Handled data, skip normal send
            generator: Handled data with async sending
        """
        if not (self.enabled and self.drop_mode_active and 
                self.cipher == custom_constants.CONFIG_AEAD):
            return False
            
        try:
            # Check if current record should be processed with OT
            should_process = self._should_process_record_with_ot()
            
            if should_process: 
                if self.first_record_aead is None:
                    # Store first record of the pair
                    self.first_record_aead = data
                    return True  # Signal to skip normal send
                else:
                    # Process the pair with OT
                    reply = self.s_otc.reply(
                        self.otc_selections[self.drop_counter], 
                        self.first_record_aead, 
                        data
                    )
                    ...
                    pickled_reply = pickle.dumps(
                        reply, protocol=pickle.HIGHEST_PROTOCOL)
                    processed_data = (
                        self._reply_begin_bytes 
                        + len(pickled_reply).to_bytes(4, 'big') 
                        + pickled_reply
                    )

                    self.data_sent += len(processed_data)
                    
                    # Return the generator directly instead of yielding
                    return sock_send_all_func(processed_data)
            else:
                self.data_sent += len(data)
            
        except Exception as e:
            if log:
                log.error(f"AEAD processing error: {e}")
        return False
    
    def get_custom_seqnum(self, write_state: "ConnectionState", normal_get_func):
        """Get sequence number with custom logic for drop mode"""
        if not (self.enabled and self.drop_mode_active):
            return normal_get_func()
        
        if (self.record_counter in self.blocks_to_modify 
            or self.record_counter - 1 in self.blocks_to_modify):
            if self.first_drop:
                # First record in pair: manually encode the current sequence number
                # without incrementing it (normally done by normal_get_func())
                encoding_length = 8  # TLS protocol requires 64-bit/8-byte sequence numbers
                writer = Writer()
                writer.add(write_state.seqnum, encoding_length)  # Encode current seqnum into 8 bytes
                seqnum_bytes = writer.bytes
                self.first_drop = False
            else:
                # Second record in pair: use normal sequence number handling
                # This will encode the SAME sequence number and increment it for next time
                seqnum_bytes = normal_get_func()
                self.first_drop = True
        else:
            seqnum_bytes = normal_get_func()
            
        # IMPORTANT: Increment record_counter here - this affects the indexing
        # in process_aead_send() which is called after this method
...
class RecordSocket(object):
    ...
    def __init__(self, sock):
        ...
        # Initialize selective dropping extension
        self.selective_drop = YouChooseRecordLayerExtension()
    ...
    def send(self, msg, padding=0):
        ...
        # Optional selective dropping for AEAD mode
        if self.selective_drop.enabled:
            aead_result = self.selective_drop.process_aead_send(
                data, self._sockSendAll)
            if aead_result is True:
                return  # Data was handled by selective dropping
            elif hasattr(aead_result, '__iter__'):  # It's a generator
                for result in aead_result:
                    yield result
                return
...
class RecordLayer(object):
    ...
    def _macThenEncrypt(self, data, contentType):
        ...
        # Get sequence number with optional selective dropping logic
        seqnumBytes = self._recordSocket.selective_drop.get_custom_seqnum(
            self._writeState, self._writeState.getSeqNumBytes
        )
    ...
    def _encryptThenSeal(self, buf, contentType):
        ...
        # Get sequence number with optional selective dropping logic for TLS 1.3
        seqNumBytes = self._recordSocket.selective_drop.get_custom_seqnum(
            self._writeState, self._writeState.getSeqNumBytes
        )
