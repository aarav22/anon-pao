...
logging.basicConfig(filename='tmp/client-logs.log', level=logging.DEBUG)

# local imports
import utils

# third party imports
from libraries.tlslite import SMTP_TLS as Client, HandshakeSettings

class MAILClient(Client):
    def __init__(
            self, hostname, port, email, password, num_chals, 
            cipher="cbc", mode="youchoose", optimize=False
        ): 
    ...
    def createMsgBody(
            self, receiver, sender, msg_type, image_path, num_chals=1, optimize=False
        ) -> str:
        subject = "PAO Challenge"
        body = "BEGIN PAO CHALLENGE"

        if msg_type == "text":
            return utils.create_text_email(
                subject, body, receiver, sender, num_msgs=num_chals, optimize=optimize
            )
        
        elif msg_type == "image":
            return utils.create_interleaved_image_email(
                subject, body, receiver, sender, image_path, 
                num_different_blocks=num_chals, optimize=optimize
    )
...
def main():
    ...
    parser.add_option("-t", "--msg-type", dest="msg_type", default="text",
                        help="text or image")
    parser.add_option("-n", "--image-path", dest="image_path", 
                        default="assets/login_image.bmp",
                        help="path to image")
